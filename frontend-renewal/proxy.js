// Production entry point for the frontend-renewal container.
//
...
//   * forwards /api/* to BACKEND_URL  (resolved at container start, fixing #225)
...
function proxyToBackend(req, res) {
  ...
  proxyReq.on("error", (err) => {
    console.error(`[api-proxy] ${req.method} ${req.url} -> ${backendUrl} failed: ${err.message}`);
    if (!res.headersSent) {
...
function proxyToNext(req, res) {
  ...
  proxyReq.on("error", (err) => {
    ...
    if (!res.headersSent) {
      ...
      res.end(JSON.stringify({ error: "Frontend not ready" }));
    } else {
      res.end();
    }
  });
