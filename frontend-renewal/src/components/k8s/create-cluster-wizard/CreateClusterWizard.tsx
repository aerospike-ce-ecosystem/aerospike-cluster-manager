...
import type { CreateK8sClusterRequest, K8sTemplateSummary } from "@/lib/types/k8s";
import {
  CE_LIMITS,
  parseCpuMillis,
  parseMemoryBytes,
...
function buildFormUpdatesFromTemplate(
  ...
  if (typeof spec.size === "number" && spec.size >= 1 && spec.size <= CE_LIMITS.MAX_NODES) {
  ...
  if (spec.monitoring && typeof spec.monitoring === "object") {
    updates.monitoring = spec.monitoring as CreateK8sClusterRequest["monitoring"];
  }
...
function cleanupPayload(form: CreateK8sClusterRequest): CreateK8sClusterRequest {
  const p: CreateK8sClusterRequest = { ...form };

  if (p.rollingUpdate) {
    const { batchSize, maxUnavailable, disablePDB } = p.rollingUpdate;
    if (batchSize === undefined && maxUnavailable === undefined && !disablePDB) {
  ...
  if (p.rackConfig && (!p.rackConfig.racks || p.rackConfig.racks.length === 0)) {
    const { namespaces, scaleDownBatchSize, maxIgnorablePods, rollingUpdateBatchSize } =
      p.rackConfig;
    if (!namespaces?.length && !scaleDownBatchSize && !maxIgnorablePods && !rollingUpdateBatchSize) {
      delete (p as { rackConfig?: unknown }).rackConfig;
    }
  }

  if (p.sidecars && p.sidecars.length === 0) delete (p as { sidecars?: unknown }).sidecars;
  ...
  if (p.bandwidthConfig && !p.bandwidthConfig.ingress && !p.bandwidthConfig.egress) {
    ...
    listK8sNamespaces()
      .then((list) => {
        ...
        setForm((prev) =>
          prev.namespace
            ? prev
            : { ...prev, namespace: list.includes("default") ? "default" : (list[0] ?? "") },
        );
      })
      .catch(() => {
  ...
  const stepError = useMemo<string | null>(() => {
    if (mode === "scratch") {
      if (step === 1) {
        ...
        if (parseMemoryBytes(lim?.memory ?? "0") < parseMemoryBytes(req?.memory ?? "0"))
    ...
    <div className="flex flex-col gap-6">
      ...
      {mode === "scratch" ? (
        step === 0 ? (
          ...
          <StepBasic form={form} namespaces={namespacesList} updateForm={updateForm} />
        ) : step === 2 ? (
        ...
        <StepBasic form={form} namespaces={namespacesList} updateForm={updateForm} templateMode />
      ) : step === 2 ? (
        <StepNamespaceStorage form={form} updateForm={updateForm} />
      ) : (
        <StepReview form={form} templateName={selectedTemplateName} />
      ...
      <div className="flex justify-between">
        ...
        {isLastStep ? (
          <Button variant="primary" onClick={handleCreate} isLoading={submitting} loadingText="Creating…">
            Create Cluster
          </Button>
        ) : (
          <Button variant="primary" onClick={goNext} disabled={Boolean(stepError)}>
            Next
          </Button>
        )}
      </div>
