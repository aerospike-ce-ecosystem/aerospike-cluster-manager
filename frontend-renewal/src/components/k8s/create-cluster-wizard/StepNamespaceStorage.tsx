...
function defaultMemoryNamespace(clusterSize: number, index: number): AerospikeNamespaceConfig {
  return {
...
export function StepNamespaceStorage({ form, updateForm }: StepNamespaceStorageProps) {
  ...
  const updateNamespace = (i: number, patch: Partial<AerospikeNamespaceConfig>) => {
    const next = namespaces.map((ns, idx) => (idx === i ? { ...ns, ...patch } : ns));
  ...
  const addNamespace = () => {
    ...
    updateForm({
      namespaces: [...namespaces, defaultMemoryNamespace(clusterSize, namespaces.length)],
    });
  };
  ...
  const legacyStorage: StorageVolumeConfig =
    (form.storage && !("volumes" in form.storage) ? form.storage : undefined) ?? {
    ...
    <Card className="flex flex-col gap-5">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Aerospike CE supports up to {CE_LIMITS.MAX_CE_NAMESPACES} namespaces per cluster.
        </p>
      </div>
        ...
        return (
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor={`ns-rf-${i}`}>Replication Factor (1-{clusterSize})</Label>
                <Input
                  id={`ns-rf-${i}`}
                  type="number"
                  min={1}
                  ...
                  onChange={(e) =>
                    updateNamespace(i, {
                      replicationFactor: Number.parseInt(e.target.value, 10) || 1,
                    })
                  }
                />
              </div>
            ...
            {storageType === "memory" ? (
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor={`ns-filesize-${i}`}>Device File Size (bytes)</Label>
                  <Input
                    id={`ns-filesize-${i}`}
                    type="number"
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              ...
              onChange={(e) =>
                updateForm({ storage: { ...legacyStorage, size: e.target.value } })
              }
              placeholder="10Gi"
            />
          </div>
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              ...
              onChange={(e) =>
                updateForm({ storage: { ...legacyStorage, mountPath: e.target.value } })
              }
            />
          </div>
        </div>
