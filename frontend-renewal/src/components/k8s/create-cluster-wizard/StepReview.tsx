...
export function StepReview({ form, templateName }: StepReviewProps) {
  ...
  return (
    <Card className="flex flex-col gap-5">
      <h2 className="text-base font-semibold text-gray-900 dark:text-gray-50">Review</h2>

      <dl className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <Field label="Name" value={form.name ?? "—"} />
        <Field label="Namespace" value={form.namespace ?? "—"} />
        <Field label="Size" value={`${form.size ?? 1} node${(form.size ?? 1) > 1 ? "s" : ""}`} />
        ...
        <Field
          label="Monitoring"
          value={form.monitoring?.enabled ? `Enabled (port ${form.monitoring.port ?? 9145})` : "Disabled"}
        />
        <Field
