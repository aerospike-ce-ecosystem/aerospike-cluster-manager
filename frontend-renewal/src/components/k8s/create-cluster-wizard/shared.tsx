...
export function Section({ title, summary, defaultOpen = false, children }: SectionProps) {
  ...
  return (
    <section className="rounded-md border border-gray-200 dark:border-gray-800">
      ...
      {open && <div className="border-t border-gray-200 p-4 dark:border-gray-800">{children}</div>}
    </section>
  );
}
...
export function KeyValueEditor({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Button type="button" variant="secondary" onClick={addEntry} className="self-start">
        + Add
      </Button>
    </div>
  );
