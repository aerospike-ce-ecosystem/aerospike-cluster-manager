...
import type { FilteredQueryRequest, FilteredQueryResponse } from "../types/query";
import type {
  AerospikeRecord,
  PkType,
  RecordListResponse,
...
export function listRecords(
  ...
  return apiGet(`/records/${encodeURIComponent(connId)}`, { query: { ...params } });
}
...
export function getRecordDetail(
  ...
  return apiGet(`/records/${encodeURIComponent(connId)}/detail`, { query: { ...params } });
}
...
export function deleteRecord(connId: string, params: RecordLookupParams): Promise<void> {
  return apiDelete(`/records/${encodeURIComponent(connId)}`, { query: { ...params } });
}
