...
function nodeStatus(node: ClusterNode): {
  ...
  if (migrating) return { label: "Migrating", variant: "warning", dot: "bg-amber-500" }
    ...
    <main className="flex flex-col gap-8">
      <header className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="secondary" onClick={() => cluster.refetch()} isLoading={cluster.isLoading}>
            Refresh
          </Button>
      ...
      {ackoInfo && (
        ...
        >
          ...
          <dl className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <Metric label="Image" value={ackoInfo.image.split(":").pop() ?? ackoInfo.image} />
            <Metric
          ...
          {cluster.isLoading ? (
            ...
            <ul className="flex flex-col divide-y divide-gray-200 dark:divide-gray-800">
              {cluster.data.nodes.map((n) => {
                ...
                return (
                  <li key={n.name} className="flex items-center gap-3 px-5 py-3">
                    <span
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <dd className="font-medium tabular-nums text-gray-900 dark:text-gray-50">{value}</dd>
    </div>
  )
}
