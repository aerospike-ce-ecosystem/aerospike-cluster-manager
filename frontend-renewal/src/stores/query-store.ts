/**
 * Query store — holds the Index Query / Scan / PK-lookup toolbar state used
 * by the Query toolbar on the record browser.
 */

import { create } from "zustand"

import { runQuery } from "@/lib/api/query"
import type { QueryPredicate, QueryResponse } from "@/lib/types/query"
...
export const useQueryStore = create<QueryState>()((set, get) => ({
  ...
  setNamespace: (ns) =>
    set({ namespace: ns, results: [], error: null, hasExecuted: false }),
  ...
  executeQuery: async (connId) => {
    ...
    try {
      const result: QueryResponse = await runQuery(connId, {
        namespace: state.namespace,
        set: state.set || null,
        predicate: state.predicate ?? null,
        selectBins: state.selectBins.length > 0 ? state.selectBins : null,
        expression: state.expression || null,
        maxRecords: state.maxRecords,
        primaryKey: state.primaryKey || null,
      })
      set({
