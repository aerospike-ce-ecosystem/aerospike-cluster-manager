...
The cluster health dashboard includes automatic split-brain detection. A warning is raised when the Aerospike-reported cluster size (from the `status.aerospikeClusterSize` field) disagrees with the expected size from the spec.

**Detection criteria:** The split-brain flag is only set when all of the following conditions are true:

- The cluster phase is `Completed` (steady state).
- The spec size is greater than zero.
- All expected pods are ready (`status.size == spec.size`).
- The Aerospike-reported cluster size differs from the spec size.

This avoids false positives during scaling, rolling restarts, and initial provisioning. When a split-brain condition is detected, the `splitBrainDetected` field in the cluster health response is set to `true`.
