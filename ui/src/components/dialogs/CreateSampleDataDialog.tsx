...
export function CreateSampleDataDialog({
  ...
  const handleSubmit = async () => {
    ...
    if (!Number.isFinite(count) || count < 1 || count > MAX_RECORDS) {
      setError(
        `Record count must be between 1 and ${MAX_RECORDS.toLocaleString()}`,
      )
      return
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Generate sample records with varied bin types (int / string / double
            / bool / list / map / geojson) for testing and exploration.
          </DialogDescription>
        </DialogHeader>
