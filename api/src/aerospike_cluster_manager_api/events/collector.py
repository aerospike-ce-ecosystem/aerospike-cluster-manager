...
class EventCollector:
    ...
    async def _publish_metrics(self) -> None:
        ...
        with _tracer.start_as_current_span(
            "asm.events.collect",
            attributes={"asm.collect.kind": "metrics", "asm.collect.connections": len(connections)},
        ):
    ...
    async def _publish_health(self) -> None:
        ...
        with _tracer.start_as_current_span(
            "asm.events.collect",
            attributes={"asm.collect.kind": "health", "asm.collect.connections": len(connections)},
        ):
            for conn in connections:
                try:
                    client = await client_manager.get_client(conn.id)
    ...
    async def _publish_k8s(self) -> None:
        ...
        with _tracer.start_as_current_span(
            "asm.events.collect",
            attributes={"asm.collect.kind": "k8s"},
        ) as span:
            try:
